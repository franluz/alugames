...
class ConsumoApi {
    ...
    fun consumoDados(endereco: String): String {
    ...
    fun buscaJogosJson ():List<Jogo>{
        ...
        val gson = Gson()
        val meusInfoJogo: List<InfoJogoJson> = gson.fromJson(json, meuTipoJogo)
        val meusJogos: List<Jogo> = meusInfoJogo.map { info ->  info.criaJogo()}
        return  meusJogos
    }

    fun buscaJogo(id: String): InfoJogo {
        val uri = "https://www.cheapshark.com/api/1.0/games?id=$id"
        val json = consumoDados(uri)
        val gson = Gson()
